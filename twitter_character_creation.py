...
for i in range(10):
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction="""You are a twitter character creator. I will give you two set of twitter data one is the real people and their tweets are about a global warming protest and one is the characters you have created for my new system. For now you will create a single character with 10 tweet history about anything they might tweet about based on their character. The characters you create should be in the following format:
        <begin example character creation>
    "aesop_id": "4083011984",
    ...
    "tweets": [10 tweets]
        <end example character creation>

        now for the role you can choose from the following but make sure the following proportion of the users is maintained:
        <begin role proportion>
        Journalist (Example: Private Account Journalist): 6.5%
        Private Person (Example: Ordinary Citizen): 25.5%
        Celebrity (Example: Famous Actor): 4%
        Media Outlet (Example: News Outlet): 13%
        Politician (Example: Patry Member): 4.5%
        Activist (Example: Environmental Activist): 3.5%
        Social Bot (Example: Unidentified Account): 1.5%
        NGO (Example: Non-government Org): 1.5%
        International Organization (Example: UN): 20.5%
        Government (Example: Government Official Account): 19%
        <end role proportion>
         The characters you create are for the following scenario:
            <begin scenario>
...
#GreenerEco
<end scenario>
Here is another file we have for scenario description:
<begin scenario>
